...
class NumpyBridge:
    '''Converts MatND messages into numpy arrays and vice versa.'''
    def __init__(self):
        self.typeDict = {
            np.uint8: "8U",
            np.int8: "8S",
    ...
    def encoding_to_dtype(self, encoding):
        for npType, encString in self.typeDict.iteritems():
            if encoding  == encString:
                return npType
        raise CvBridgeError, "Bad encoding type: %s" % encoding

    def dtype_to_encoding(self, dtype):
        for npType, encString in self.typeDict.iteritems():
            if dtype == npType:
                return encString
        raise CvBridgeError, "The type %s is not suppored" % dtype
    
    def matmsg_to_numpy(self, img_msg, desired_encoding = "passthrough"):
        retval =  np.ndarray(buffer = img_msg.data,
                             dtype = self.encoding_to_dtype(img_msg.encoding),
                             shape = img_msg.sizes)
        
        if (desired_encoding <> "passthrough" and
            desired_encoding <> img_msg.encoding):
            # Do a data conversion if requested
            retval = np.array(retval,
                              dtype = self.encoding_to_dtype(desired_encoding))
        return retval
        

    def numpy_to_matmsg(self, arr, desired_encoding = "passthrough"):        
        # Change the datatype if necessary
        convertedArr = arr
        if (desired_encoding <> "passthrough" and
            desired_encoding <> self.dtype_to_encoding(arr.dtype)):
            convertedArr = np.array(arr,
                                    dtype=self.encoding_to_dtype(desired_encoding))
        

        # Pack into the message
        retval = sensor_msgs.msg.MatND()
        retval.data = str(convertedArr.data)
        retval.encoding = self.dtype_to_encoding(convertedArr.dtype)
        retval.sizes = convertedArr.shape
